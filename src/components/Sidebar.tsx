...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [isThemeCollapsed, setIsThemeCollapsed] = useState(() => {
    const saved = localStorage.getItem('pisaka-theme-collapsed');
  ...
  const [isIntegrationsCollapsed, setIsIntegrationsCollapsed] = useState(() => {
    const saved = localStorage.getItem('pisaka-integrations-collapsed');
    return saved !== null ? saved === 'true' : true; // Default to collapsed
  });

  const toggleIntegrationsCollapse = () => {
    setIsIntegrationsCollapsed(prev => {
      const next = !prev;
      localStorage.setItem('pisaka-integrations-collapsed', String(next));
      return next;
    });
  };
      ...
      <div className={cn(
        ...
        {!isCollapsed && (
          <div className="border-t border-zinc-900 pt-2 mt-1 w-full">
            <button
              onClick={toggleIntegrationsCollapse}
              className="flex items-center justify-between w-full text-[9px] font-bold text-zinc-500 uppercase tracking-wider mb-2 hover:text-zinc-300 transition-colors outline-none"
            >
              ...
              <ChevronDown className={cn("w-3 h-3 transition-transform duration-200", isIntegrationsCollapsed && "-rotate-90")} />
            </button>
